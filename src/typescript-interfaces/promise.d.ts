...
declare class Promise<F,R> {
  ...
  then<F2,R2>(onFulfilled : (F) => Promise<F2,R2>,
              onRejected ?: (R) => void)
      : Promise<F2,R2>;
  catch(catchFn : (rejectObj: R) => void) : Promise<F,R>;

  static resolve<F,R>(thenable : Thenable<F,R>) : Promise<F,R>;
  static resolve<F,R>(fulfillObj : F) : Promise<F,R>;
  static reject<F,R>(rejectObj : R) : Promise<F,R>;
  static all<F,R>(...args: Thenable<F,R>[]) : Promise<F,R>;
  static race<F,R>(...args: Thenable<F,R>[]) : Promise<F,R>;
}
